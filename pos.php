					...
					<div class="mb-4">
						<div class="row">
							<div class="form-group col">
								...
								<select name="customer_id" id="customer_id" class="custom-select browser-default select2">
									<option value="0" data-discount="0" selected="">Normal</option>
									<?php 
									$customer = $conn->query("SELECT * FROM customer_list ORDER BY customer_type ASC");
...
<script>
	...
	function calculate_total() {
		...
		var discountPercent = parseFloat($('#customer_id option:selected').data('discount')) || 0;
		var discountAmount = subTotal * (discountPercent / 100);

		var totalAfterDiscount = subTotal - discountAmount;

		$('#list .sub-total').html(parseFloat(subTotal).toLocaleString('en-US', { style: 'decimal', maximumFractionDigits: 2, minimumFractionDigits: 2 }));
		$('#list .discount').html(parseFloat(discountAmount).toLocaleString('en-US', { style: 'decimal', maximumFractionDigits: 2, minimumFractionDigits: 2 }));
		$('#list .total-amount').html(parseFloat(totalAfterDiscount).toLocaleString('en-US', { style: 'decimal', maximumFractionDigits: 2, minimumFractionDigits: 2 }));

		$('[name="tamount"]').val(totalAfterDiscount);
	}
